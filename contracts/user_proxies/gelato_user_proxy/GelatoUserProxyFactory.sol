...
} from "./interfaces/IGelatoUserProxyFactory.sol";
...
contract GelatoUserProxyFactory is IGelatoUserProxyFactory {
    ...
    ) external payable override returns (GelatoUserProxy userProxy) {
        userProxy = create();
        if (_tasks.length != 0)
            _submitTasks(userProxy, _provider, _tasks, _expiryDates);
    }
        ...
        returns (GelatoUserProxy userProxy)
    {
        ...
        userProxy = new GelatoUserProxy{salt: salt, value: msg.value}(
            msg.sender,
            gelatoCore
        );
        require(
    ...
    function createTwoExecActionsSubmitTaskCycle(
        ...
        if (_tasks.length == 0)
            revert(
                "GelatoUserProxyFactory.createTwoExecActionsSubmitTaskCycle: 0 _tasks"
            );
        _submitTaskCycle(userProxy, _provider, _tasks, _expiryDate, _cycles);
    }
    ...
    function createTwoExecActionsSubmitTaskChain(
        ...
        if (_tasks.length == 0)
            revert(
                "GelatoUserProxyFactory.createTwoExecActionsSubmitTaskChain: 0 _tasks"
            );
        _submitTaskChain(
            userProxy,
            _provider,
    ...
    function isGelatoUserProxy(address _proxy)
        external
        view
        override
        returns (bool)
    {
        return
            userByGelatoProxy[GelatoUserProxy(payable(_proxy))] != address(0);
    }
    ...
    function _execActions(
        GelatoUserProxy _userProxy,
        Action[] calldata _actions
    ) private {
        try _userProxy.multiExecActions(_actions) {} catch Error(
            string memory err
        ) {
            revert(
                string(
                    abi.encodePacked(
                        "GelatoUserProxyFactory._execActions:",
                        err
                    )
                )
            );
    ...
    function _submitTasks(
        ...
        try
            _userProxy.multiSubmitTasks(_provider, _tasks, _expiryDates)
        {} catch Error(string memory err) {
            revert(
                string(
                    abi.encodePacked(
                        "GelatoUserProxyFactory._submitTasks:",
                        err
                    )
                )
            );
    ...
    function _submitTaskCycle(
        ...
        try
            _userProxy.submitTaskCycle(_provider, _tasks, _expiryDate, _cycles)
        {} catch Error(string memory err) {
            revert(
                string(
                    abi.encodePacked(
                        "GelatoUserProxyFactory._submitTaskCycle:",
                        err
                    )
                )
            );
    ...
    function _submitTaskChain(
        ...
        {} catch Error(string memory err) {
            revert(
                string(
                    abi.encodePacked(
                        "GelatoUserProxyFactory._submitTaskChain:",
                        err
                    )
                )
            );
